...
var ajax_ = function(obj) {
        ...
        var userId = localStorage.getItem('userMobile') || '';
        //step2 step3:连接 和 发送
        if (obj.type == 'GET') {
           ...
           // xhr.setRequestHeader('activityCode', activityCode);
...
//ajax 二次封装
var ajax = function(url, obj, success, error) {
    console.log({
    ...
    ajax_({
        ...
        data: {
         ...
         //request:JSON.stringify(obj) //调试阶段非加密保文
        },
        success: function(data, b) {
            try{
                var data_ = JSON.parse(data);
                data_ = JSON.parse(data_.body);
                success(data_);
            }
            catch(e){
                if(data_['retCode'] == 9002){ //账号在其他地方登陆
                       myApp.alert(data_['retMsg'],function(){
                         localStorage.removeItem('userMobile');
                         localStorage.removeItem('token');
                         location.href = location.origin+location.pathname+'?type=login';
                       })
                }else{
                       myApp.alert('返回报文格式有误：</br><span style="word-break:break-all">'+data+'</span>')  
                }
 ...
 * fn1:成功回调函数
 * fn2:失败回调函数
 * 使用方式：ajax(URL, DATA, function(data) {}, function(data) {})
*/