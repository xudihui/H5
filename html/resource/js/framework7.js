...
(function () {
    ...
    window.Framework7 = function (params) {
		...
		app.ajaxLoad = function (config){
			...
			/* mainView.router.load({  //此处切记，主视图初始化的变量名必须是mainView，不然跳转不了
				url: "smk-top-tool.html",
				context:self.data('topTool')
			});	 */ 
		}	
		...
		app.ajaxGetData = function(config){
			...
			var date = b.slice(0,-2);	
            
			var data = config.data || function(){return{}},
			url = config.url,
			view = config.view,
            Tips = config.tips || '列表加载完毕!',
			fn = config.callback || function(){},
			el = '.page[data-page="' + config.target + '"]',
			el = $$(el),	
			current = el.find('.scroll_content_current input.currentPage'),
			currentPage = parseInt( current.val() || -1),
			max = el.find('.scroll_content_current input.maxPage'),
			pageno = config.pageno || 'PageIndex',//请求页码字段
			wrap = config.wrap || $$(el).find('.list-block ul');
			if(AJAXDATE == date){ //设置全局变量，当滚动惯性的时候，点击切换标签，不进行数据加载，把毫秒转换成秒，同一秒发送的请求，只发送一次。
				fn(data);//重置翻页的loading
				return
			}
            console.log('初始页码:' + current.val())
			AJAXDATE = date;			
			data = data();
			if( data.currentPage ){
				data[pageno] = data.currentPage; 
			}
			else{//返回字段中如果没有当前页面则自己加1
				
				data[pageno] = ++currentPage;
			}
            ajax(url,data,function(data_,tips){
                        console.log(data_)
                         ...
                         */
                        current.val(parseInt(currentPage));
                        if(data_['data'].length == 0 && currentPage==0){
                            data_['Result'] = false;
                        }
                        else{
                            data_['Result'] = true
                        }
                        max.val(data_['totalpage']);    
                        html = SMK.templates[view](data_);

                        // 添加新条目
                        wrap.append(html);
                        if(data_['data'].length < data.PageSize && data_['data'].length > 0){
                            console.log(tips)
                            $$(el).find('.scroll_content_current .list-record').append('<p class="tips_nomore" style="text-align:center;padding-bottom:30px;color:#aaa">' + Tips + '</p>');
                            max.val(currentPage)
                        }
                      
                        var empty = el.find('.none').parent();
                        if(empty.length>1){
                           // empty.eq(0).remove();  //在下拉正在请求数据阶段，迅速切换到tab处（目前仅交易记录能达到快速切换，其它都有二级菜单切换），会触发二次请求，如果已有一个空列表，则删除旧的空列表。触发二次请求是因为第一次请求的内容触发了scroll事件。
                        }                      
                        fn(data_);//把data参数传递给回调函数，判断返回数据是否为空
		...
		app.ajaxNextPage = function (config){
			console.log("ajaxNextPage config is:");
			console.log(config);
			...
			var data = config.data || function(){return{}},
				...
				el = '.page[data-page="' + config.target + '"]';
				el = $$(el);
				var current = el.find('.scroll_content_current input.currentPage');  //当前tab的页码
				var max = el.find('.scroll_content_current input.maxPage');   //当前tab的总页码
				console.log(data())
				console.log(current.val()+"currentPage maxPage is"+max.val())
                var lastPage = function(){
                          if($$(el).find('.scroll_content_current .tips_nomore').length == 0){
				...
				$$(el).find('.infinite-scroll').on('infinite', function () {
					// 上次加载的序号
                  current = el.find('.scroll_content_current input.currentPage');  
                  max = el.find('.scroll_content_current input.maxPage');   //当前tab的总页码
				  var currentPage = parseInt(current.val() || 1);
					// 最多可加载的条目
				  var maxPage = parseInt(max.val() || -1);//把不返回总页码的接口总页数置为-1
				  // 如果正在加载，则退出
				  console.log(loading)
				  console.log(222)
				  if (loading) return;
				  ...
				  // 模拟200ms的加载过程
				  //setTimeout(function () {
				  console.log(currentPage+"currentPage maxPage is"+maxPage)
				    ...
				    app.ajaxGetData({
						...
						wrap:el.find('.scroll_content_current .list-record') || $$(el).find('.list-block ul'),
						callback:function(data_){
                            loading = false;
                            if(data_){
                              fn(data_)
                            }
                        },//loading传值进入ajaxGetData之后不能反向映射到外面来，所以使用回调函数，为翻页也添加回调fn函数，并传递参数data
						url:url, //请求地址 String
						pageno:config.pageno || 'PageIndex',//请求页码字段
					});	
        ...
        ************   Modals   ************
        ...
        app.alertButton = function (text, title, button,callbackOk) {
            if (typeof title === 'function') {
                callbackOk = arguments[1];
                title = undefined;
            }
            return app.modal({
                text: text || '',
                title: typeof title === 'undefined' ? app.params.modalTitle : title,
                buttons: [ {text: button, bold: true, onClick: callbackOk} ]
            });
        };
        ...
        app.prompt = function (text, title, callbackOk, callbackCancel) {
            ...
            return app.modal({
                text: text || '',
                title: typeof title === 'undefined' ? app.params.modalTitle : title,
                afterText: '<div class="input-field"><input type="password" maxlength="6" class="modal-text-input"></div>',
                buttons: [
                    {
        ...
        app.hideLogin = function () {
            $('span.preloader-indicator-overlay, span.preloader-indicator-modal').remove();
        };
        // Action Sheet
        ...
        ************   Handle Browser's History   ************
        ...
        app.initPushState = function () {
            ...
            function handlePopState(e) {
                ...
                if($('.page-from-left-to-center').length>0 && stateUrl !== view.url){  //点击浏览器物理回退按钮时，执行回退动画的同时不进行连续回退；当使用mainView.router.back()会主动触发页面动画，先判断当前页和返回页不一致时执行pushState，否则不执行
                	 var page_current = $('.view-main').attr('data-page');
                	 history.pushState({}, "", "#!/"+page_current+".html");  //这里的回退地址紧跟项目路径
               	     return;
               	}
        ...
        $.ajax = function (options) {
            ...
            function fireAjaxCallback (eventName, eventData, callbackName) {
                ...
                if (callbackName) {
                	...
                	//踢下线功能
                	if(callbackName == 'success' && a[3].retCode == '9205'){  //请求成功，并且retcode为9205表示被踢下线，执行重刷页面
                		var appId = localStorage.getItem("appId")||'';
                		localStorage.clear();
                		myApp.alert('您的账户已在另一个微信上登录，如非本人操作，密码可能已经泄露，建议尽快修改密码', '市民卡提示!', function () {
                			location.href='https://open.weixin.qq.com/connect/oauth2/authorize?appid='+appId+'&redirect_uri='+location.href.split('?')[0]+'&response_type=code&scope=snsapi_base&state=1#wechat_redirect';
					    });
                		return; //此处即使不return 也不会执行后续代码，因为页面的href被刷了.
                	}
            ...
            // Create POST Data
            var postData = null,
                version = $('html').attr('version')||'', //版本号,写在首页html标签里面
                smId = localStorage.getItem('smid')||'', //smId,判断用户是否被踢下线
                openId = localStorage.getItem('openId')||'', //openId,判断用户是否被踢下线
                userId = localStorage.getItem('userId')||''; //userId,判断用户是否被踢下线
            xhr.setRequestHeader('channel','wechat');      
            xhr.setRequestHeader('version',version);            
            xhr.setRequestHeader('smId',smId);
            ...
            if (options.timeout > 0) {
                xhrTimeout = setTimeout(function () {
                	   // alert(FLAG)
					    ...
					    var requestUrl = xhr.requestUrl;
					    if(options.timeoutFn){ //超时回调，如有执行回调，直接返回     by xudihui2016/07/22
					    	 var fn = options.timeoutFn;
					    	 fn();
					    	 return;
					    }
						if(requestUrl.indexOf('bindJudge')==-1 && FLAG){
						  showTips('网络好像出问题了！请您重新尝试！');
						}
						else if(requestUrl.indexOf('bindJudge')>-1){
							  showTips('加载失败，请稍候再试');
