				...
				$$(el).find('.infinite-scroll').on('infinite', function () {
				  ...
				  console.log(currentPage+"currentPage maxPage is"+maxPage)
					if (currentPage >= maxPage && maxPage != -1) {
						console.log(currentPage+'##'+maxPage+'$$'+typeof(maxPage))
					  ...
					  $$(el).find('.infinite-scroll').append('<p class="tips_nomore" style="text-align:center;padding-bottom:30px;color:#aaa">' + tips + '</p>');
				    ...
				    app.ajaxGetData({
						...
						wrap:config.wrap || $$(el).find('.list-block ul'),
						callback:function(data){loading = false;if(data){fn(data)}},//loading传值进入ajaxGetData之后不能反向映射到外面来，所以使用回调函数，为翻页也添加回调fn函数，并传递参数data
						url:url, //请求地址 String
						pageno:config.pageno || 'pageno',//请求页码字段
					});	
        ...
        app.initPushState = function () {
            ...
            function handlePopState(e) {
                ...
                if($('.page-from-left-to-center').length>0 && stateUrl !== view.url){  //点击浏览器物理回退按钮时，执行回退动画的同时不进行连续回退；当使用mainView.router.back()会主动触发页面动画，先判断当前页和返回页不一致时执行pushState，否则不执行
                	 ...
                	 history.pushState({}, "", "#!//weixin/public/"+page_current+".html");  //这里的回退地址紧跟项目路径
               	     return;
               	}
